...
const PLANS = [
  ...
  },
  {
    ...
    name: "Enterprise",
    price: 299,
    desc: "Full-service for serious operators.",
    features: [
          ...
          <div className="flex-1 rounded-[15px] p-8 flex flex-col overflow-y-auto" style={{ background: "rgba(83,83,83,0.25)", border: "1px solid rgba(255,255,255,0.07)" }}>
              ...
              <div className="flex items-center gap-1 p-1 rounded-[10px]" style={{ background: "rgba(255,255,255,0.07)" }}>
                {(["monthly", "annual"] as const).map((b) => (
                  <button
                    ...
                    style={{
                      background: billing === b ? "rgba(148,82,232,0.4)" : "transparent",
                      color: billing === b ? "white" : "rgba(255,255,255,0.45)",
                    }}
                  >
                    {b}
                    {b === "annual" && <span className="ml-1 text-[10px] font-bold" style={{ color: "#10B981" }}>-20%</span>}
                  </button>
                ))}
              </div>
            </div>
            ...
            <div className="grid grid-cols-3 gap-4 flex-1">
              {PLANS.map((plan) => {
                ...
                const price = billing === "annual" ? Math.round(plan.price * discount) : plan.price;
                return (
                  <button
                    key={plan.id}
                    type="button"
                    ...
                    style={{
                      background: plan.highlight && isSelected
                        ? "rgba(148,82,232,0.20)"
                        : isSelected
                        ? "rgba(148,82,232,0.12)"
                        : "rgba(255,255,255,0.04)",
                      border: isSelected
                        ? "1.5px solid rgba(148,82,232,0.6)"
                        : plan.highlight
                        ? "1px solid rgba(148,82,232,0.25)"
            ...
            <div className="flex items-center justify-between mt-6 pt-5 border-t border-white/10">
              <Link href={`/onboarding/company-type?country=${country}&type=${type}`} className="flex items-center gap-2 px-5 py-2.5 rounded-[10px] text-sm text-white/60 hover:text-white transition-colors cursor-pointer" style={{ background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.10)" }}>
